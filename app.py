...
def identify_lichen_species(image_bytes, mime_type):
    ...
    Gemini performs a forced-choice classification and MUST
    choose exactly one species from Lichenary's fixed
    pollution database.

    Gemini is NOT asked to calculate pollution.
    ...
    # ---------------------------------------------------------
    # The database itself defines the complete set of species
    # Gemini is allowed to return.
    # ---------------------------------------------------------

    species_list = list(LICHEN_POLLUTION_DATABASE.keys())

    prompt = """
You are identifying a lichen from a photograph.

IMPORTANT:

This is a FORCED-CHOICE CLASSIFICATION task.

You MUST choose exactly ONE species from the supplied
Lichenary species list.

Do NOT invent a species.
Do NOT return a species outside the supplied list.
Do NOT return UNKNOWN.
Do NOT return "not sure".
Do NOT return a description.
Do NOT return a confidence score.
Do NOT return a pollution score.
Do NOT explain your answer.

Compare the visible morphology in the photograph with the
species in the supplied list and select the species that
MOST CLOSELY resembles the photographed lichen.

Choose the closest available match from the list even if
the photograph is imperfect, the lichen is partially
obscured, or the image quality is not ideal.

Your response must be exactly one species name from the
supplied list.
...
""" + "\n".join(
    ...
    #
    # Gemini's allowed output values are literally the keys
    # from LICHEN_POLLUTION_DATABASE.
    # ---------------------------------------------------------
    ...
    for attempt in range(3):
        ...
        try:
            ...
            species = (response.text or "").strip()
            ...
            #
            # Even though the Gemini response schema restricts
            # the model to our species list, the application
            # still independently verifies the returned value.
            # -------------------------------------------------

            if species not in LICHEN_POLLUTION_DATABASE:
                raise RuntimeError(
                    f"Gemini returned an invalid species: {species}"
                )
    ...
    try:
        ...
        if not species:
            ...
            return jsonify({
                'success': False,
                'message': (
                    'The AI could not identify a lichen species '
